...
class ARKitController {
  ...
  Future<Matrix4?> viewMatrix() async {
    final viewMatrix =
        await _channel.invokeListMethod<double>('viewMatrix');
    return viewMatrix != null
        ? _matrixConverter.fromJson(viewMatrix)
        : null;
  }
  ...
  /// Returns the file path to the saved HDR image in OpenEXR format.
  Future<String> captureHDRImage() async {
    final result = await _channel.invokeMethod<String>('captureHDRImage');
    return result!;
  }
